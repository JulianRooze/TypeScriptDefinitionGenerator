...
namespace TypeScriptDefinitionGenerator
{
  ...
  internal class InterfaceDefinitionsGenerator
  {
    ...
    private string GetPropertyComment(TypeScriptProperty p)
    {
      ...
      var propertyComment = _propertyCommenter.GetPropertyComment((dynamic)p.Type);
    ...
    {
      _sb.AppendLine("{0}{3} : {1}{2}; {4}", p.Property.Name, _moduleNameGenerator.GetModuleName((dynamic)p.Type), _typeNameGenerator.GetTypeName((dynamic)p.Type), HandleOptional(p.Type), GetPropertyComment(p));
    }
